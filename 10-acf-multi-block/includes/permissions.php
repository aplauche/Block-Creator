 ...
 */
add_filter( 'acf/settings/show_admin', 'fsdacf_custom_permissions' );
 ...
 */
add_filter( 'block_editor_settings_all', 'fsdacf_restrict_locking_ui', 10, 2 );
 ...
 * @since 0.1.3
 */
function fsdacf_restrict_locking_ui( $settings ) {
	$settings['canLockBlocks'] = fsdacf_custom_permissions();
	return $settings;
}
 ...
 * @since 0.1.2
 */
function fsdacf_custom_permissions() {
	// If our user can manage site options.
	if ( current_user_can( 'manage_options' ) ) {

		// Short circuit the email logic - comment out to check against email too
		return true;
